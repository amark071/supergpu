...
这里就可以看出选择$2\times2$ 主元的精妙之处：
因为整个过程涉及到需要求矩阵的逆，
因此矩阵行列式的数值就关乎数值稳定性的结果：
如果其行列式值过小，则会导致数值溢出等的问题。

观察$2\times2$ 主元的行列式值为 
$\Delta_k=s^{(k)}_{kk}s^{(k)}_{kk}-s^{(k)}_{pk}s^{(k)}_{pk}$，注意到
出现$2\times2$ 主元说明前序 $1\times1$ 判定已经全部失败，于是就有：
$$|s^{(k)}_{kk}|<\frac{\gamma}{\sigma_p}(s^{(k)}_{pk})^2,\qquad |s^{(k)}_{pp}|<\gamma\sigma_p.$$
带入到行列式的计算中，得到：$|\Delta_k|<(1-\gamma^2)(s^{(k)}_{pk})^2$，由于阈值 $\gamma$ 已经选定，
所以当列主元不为极小的值时，行列式的绝对值不会过小，保证了数值稳定性。

但是对于不正定的矩阵仍然会存在找不到列主元的情况：即当前列全零，
这样的情况下该选主元策略也会失效。
为了处理这种情况，我们类似于非对称的处理将其移动到最后一个主元位置
